    ...
    loop {
        ...
        match event {
            ...
            PtyInstruction::NewTab(
                ...
                log::info!("PtyInstruction::NewTab: spawning terminals for tab {}", tab_index);
...
impl Pty {
    ...
    pub fn spawn_terminal(
        ...
        let (terminal_id, reader, child_pid): (u32, Box<dyn Read + Send>, Option<u32>) = self
            .bus
            .os_input
            .as_mut()
    ...
    pub fn spawn_terminals_for_layout(
        ...
        log::info!("spawn_terminals_for_layout: {} tiled + {} floating panes created, sending ApplyLayout",
            new_tab_pane_ids.len(), new_tab_floating_pane_ids.len());
        self.bus
            .senders
    ...
    ) -> Result<Option<(u32, bool, Option<RunCommand>, Result<Box<dyn Read + Send>>)>> {
        // terminal_id,
        // starts_held,
        // command
        // successfully opened
                        ...
                        .reserve_terminal_id()
                    {
                        Ok(terminal_id) => {
                            Ok(Some((
                                ...
                                starts_held,
                                Some(command.clone()),
                                Ok(Box::new(std::io::empty()) as Box<dyn Read + Send>), // placeholder, never used for held panes
                            )))
                        },
                        Err(e) => Err(e),
                    }
    ...
    pub fn rerun_command_in_pane(
        ...
        match pane_id {
            PaneId::Terminal(id) => {
                ...
                let (reader, child_pid): (Box<dyn Read + Send>, Option<u32>) = self
                    .bus
                    .os_input
                    .as_mut()
                    .context("no OS I/O interface found")
...
pub fn get_default_shell() -> PathBuf {
    zellij_os::shell::get_default_shell()
}
